...
// import { Droplet, AlertCircle, CheckCircle, Loader, X, Settings, Layers, MapPin, Gauge, HardDrive } from "lucide-react"
// import {
...
//         dimensions: {
//           ...prev.dimensions,
//           coneAngle: selectedPreset.coneAngle ? selectedPreset.coneAngle.toString() : prev.dimensions.coneAngle,
//           ullage: selectedPreset.ullage ? selectedPreset.ullage.toString() : prev.dimensions.ullage,
//         },
//       }))
...
//       offsetDepth:
//         deviceNeedsOffset(formData.deviceType) && formData.offsetDepth !== "" ? Number(formData.offsetDepth) : null,
//       bulkDensity:
...
//                     onChange={(e) => setFormData((prev) => ({ ...prev, selectedDeviceId: e.target.value }))}
//                     className="w-full px-4 py-3 bg-white/70 dark:bg-gray-700/70 backdrop-blur-sm border border-gray-200 dark:border-gray-600 rounded-xl shadow-sm transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent hover:border-blue-300"
//                   >
...
//                         {device._isCurrentlyAssigned ? " (Currently Assigned)" : ""}
//                       </option>
//                     ))}
//                   </select>
...
//                     <button
//                       type="button"
//                       onClick={() => setFormData((prev) => ({ ...prev, selectedDeviceId: "" }))}
//                       className="mt-2 text-sm text-red-600 hover:text-red-800 dark:text-red-400 dark:hover:text-red-200 transition-colors duration-200"
//                     >
      ...
      <div className="bg-white/90 dark:bg-gray-800/90 backdrop-blur-xl border border-white/20 dark:border-gray-700/50 shadow-2xl rounded-2xl w-full max-w-6xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="bg-white/90 dark:bg-gray-800/90 p-6 flex-shrink-0 flex justify-end items-center border-t border-white/20 dark:border-gray-700/50 space-x-3">
          ...
          <button
            ...
            type="submit"
            disabled={isLoading}
            className={`px-6 py-3 font-bold rounded-xl transition-all duration-300 flex items-center justify-center space-x-2 ${
              isLoading
